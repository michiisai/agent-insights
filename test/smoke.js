...
const spanIds  = (db) => db.prepare("SELECT json_extract(raw, '$.span.spanId') AS id FROM raw_spans").all().map(r => r.id);
const rawBytes = (db) => db.prepare('SELECT COALESCE(SUM(LENGTH(raw)), 0) AS b FROM raw_spans').get().b;
...
async function retentionChecks() {
  ...
  try {
    ...
    check(rawBytes(tighter.getDb()) <= 2 * PAD * 2,
