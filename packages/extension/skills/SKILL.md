...
- Treat the rules below as intent routing, not independent keyword triggers. The most specific intent wins. In particular, a request about one or more specific sessions routes to `agent-insights_getSessionSummary`; mentions of those sessions' errors, tokens, tools, duration, or performance do not also trigger the aggregate error, token/tool, slow-span, service, or trace tools. Only `getSessionSummary`, `getSessionMessages`, and `getTrace` (via a traceId from the session's timeline) can be scoped to a session at all — every other tool takes only a service or time window, so it aggregates across unrelated sessions and cannot answer a question about a specific one. Call those only when the user explicitly asks for the aggregate.
